...
public class Runtime extends RuntimeAlgorithm
{
	...
	public void run(ArrayList<DataCenter> aDatacenters, Workflow aWorkflow) throws DistributionException
	{
		...
		while(fTasks.size() > 0)
		{
			...
			//System.out.println("lReadyTasks---->"+lReadyTasks.size());
			...
			for(int i = 0; i < lReadyTasks.size(); i++)
			{
				...
				}// end for (used datacenters) --> choice of adequate datacenter for task scheduling
						...
						if(lTargetCenter.freeSpace() < lNewData.get(j).getSize())
						{
							...
							lScheduleChanges = bestCenter + " executing " + lReadyTasks.get(i) + 
												" and sending the result (" + lNewData.get(j).getSize() + ") to " 
																	+ lTargetCenter + " required a reschedule\r\n";
