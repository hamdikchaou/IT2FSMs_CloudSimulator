...
public class FcmPSOPartition {
	...
	private static double fuzziness=10;
	...
	private void Fuzzy() 
	{
		...
		for(int i=0;i<num_cluster;i++)
		{
			...
			FuzzyCenter data = new FuzzyCenter(center, nameDC, sizeDC);//storage_size
			...
			for(int j=0;j<num_cluster;j++){
				...
				if(center.get(j).size<=dc_size){
					...
					if((center.get(j).dep - cluster_point.get(j).dep)<epsilon){
						cond=false;
						System.out.println(center.get(j).name+" diff(<eps)="+(center.get(j).dep - cluster_point.get(j).dep));
					}
					else{
						System.out.println(center.get(j).name+" diff(>=eps)="+(center.get(j).dep - cluster_point.get(j).dep));
						cond=true;
					...
					System.out.println(center.get(j).name+" size>dc_size");
					cond=false;
				}
