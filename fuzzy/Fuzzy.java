...
public class Fuzzy {
		...
		public Fuzzy() {
			fuzzy_data=new Fuzzy_data(Fuzzy_parametre.getFuzziness(),Fuzzy_parametre.getNum_cluster(), 
					Fuzzy_parametre.getEssaie_number(), Fuzzy_parametre.getValmin_x(),Fuzzy_parametre.getValmax_x(),
					Fuzzy_parametre.getValmin_y(), Fuzzy_parametre.getValmax_y());
		...
		public void init()
		{
			for(int i=0;i<Fuzzy_parametre.getEssaie_number();i++){
				for(int j=0;j<Fuzzy_parametre.getNum_cluster();j++){
					matrix[i][j]=euclidien_distance(fuzzy_data.getData_point(i).getX(),fuzzy_data.getData_point(i).getY(),
							fuzzy_data.getCluuster_point(j).getX() ,fuzzy_data.getCluuster_point(j).getY());
				}
			}
		}
		...
		public List<Point>[] cluster()
		{
			...
			for(int i=0;i<Fuzzy_parametre.getEssaie_number();i++){
				...
				//System.out.println("---/////cluster[max]="+fuzzy_data.getData_point(i));
			}
			return cluster;
		}	
}